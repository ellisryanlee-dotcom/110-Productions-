...
**What:** A popular community plugin that makes the agent work like a senior engineer instead of sprinting to code: it plans first, works in an isolated environment, writes tests before code, and reviews its own work in two passes — once for spec match, once for code quality.
...
**Original example to invent:** Source cites HVAC/marketing production tools generically. Writers should invent a concrete production build and show an edge case the plan-test-review loop catches.
